...
public class Handler implements HttpHandler {
	...
	public void handle(HttpExchange he) throws IOException {
		...
		while((line = br.readLine()) != null) {
			...
			else if(line.contains("static/") && line.contains(".js")) {
			 	int firstIndex = line.indexOf("static/");
			 	int secondIndex = line.indexOf(".js") + 3;
			 	System.out.println("<script>" + getCSSCode(line.substring(firstIndex, secondIndex)) + "</script>");
			 	os.write(("<script>" + getCSSCode(line.substring(firstIndex, secondIndex)) + "</script>").getBytes());
			}
			else {
	...
	public String getCSSCode(String x) throws IOException {
		Scanner scan = new Scanner(new File("html/" + x));
		String ret = "";
